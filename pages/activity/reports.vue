<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.activity.reports') }}</h1>
    <div class="grid grid-cols-12 gap-[24px] ">
      <div class="col-span-12 xl:col-span-9" v>
...
useHead({
  title: $t('page-titles.activity.reports')
})
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getActivityPostBySlug({slug}, 'reports', page)
