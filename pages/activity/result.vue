<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.activity.result') }}</h1>
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getActivityPostBySlug({slug}, 'result', page)
    if (data && data?.value) {
...
useHead({
  title: $t('page-titles.activity.result')
})
