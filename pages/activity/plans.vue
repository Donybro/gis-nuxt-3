<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.activity.plans') }}</h1>
...
useHead({
  title: $t('page-titles.activity.plans')
})
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getActivityPostBySlug({slug}, 'plans', page)
    if (data && data?.value) {
