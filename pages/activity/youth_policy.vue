<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.activity.youth-policy') }}</h1>
...
useHead({
  title: $t('page-titles.activity.youth-policy')
})
const itemsList = ref([])
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getActivityPostBySlug({slug}, 'policy', page)
    if (data && data?.value) {
