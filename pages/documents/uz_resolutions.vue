<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.documents.uz-resolutions') }}</h1>
...
useHead({
  title: $t('page-titles.documents.uz-resolutions')
})
const itemsList = ref([])
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getDocumentsBySlug(slug, 'uz_resolutions', undefined, undefined, page)
    if (data && data?.value) {
