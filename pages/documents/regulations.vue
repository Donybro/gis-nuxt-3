<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.documents.regulations') }}</h1>
...
useHead({
  title: $t('page-titles.documents.regulations')
})
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getDocumentsBySlug(slug, 'regulations', undefined, undefined, page)
    if (data && data?.value) {
