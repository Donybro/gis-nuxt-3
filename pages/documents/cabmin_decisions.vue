<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.documents.cabmin-decisions') }}</h1>
...
const {t: $t} = useI18n()
useHead({
  title: $t('page-titles.documents.cabmin-decisions')
})
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getDocumentsBySlug(slug, 'cabmin_decision', undefined, undefined, page)
    if (data && data?.value) {
