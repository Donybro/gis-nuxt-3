<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.events.teaching') }}</h1>
...
useHead({
  title: $t('page-titles.events.teaching')
})
const itemsList = ref([])
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getEventsDataBySlug(slug, 'teaching', page)
    if (data && data?.value) {
