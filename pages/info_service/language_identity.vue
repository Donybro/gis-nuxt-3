<template>
  <section class="container_wrapper">
    <h1 class="section_title"> {{ $t('page-titles.info-service.lang-identity') }}</h1>
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getInfoServiceBySlug({submenu_slug: slug}, 'identy', page)
    if (data && data?.value) {
...
useHead({
  title: $t('page-titles.info-service.lang-identity')
})
