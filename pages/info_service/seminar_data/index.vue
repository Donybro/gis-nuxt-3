<template>
  <div class="container_wrapper">
    <h1 class="section_title mb-[16px]">{{ $t('page-titles.info-service.seminar-data') }}</h1>
    <div class="grid grid-cols-12 gap-[24px]">
      <div class="col-span-12 xl:col-span-9">
        <div class="flex flex-col">
          <ItemsList v-for="item in itemsList">
            <NuxtLink :to="`/info_service/seminar_data/${item.id}`">{{ item.title }}</NuxtLink>
          </ItemsList>
        </div>
        <PaginationCrumbs v-if="allPagesCount > 1" :allPagesCount="3" :currentPage="currentPage"
...
import {getInfoServiceAdditionalInfoBySlug} from "../../../api/getInfoServiceData";
...
async function getItems(slug = route.path, page = 1) {
  try {
    const {data} = await getInfoServiceAdditionalInfoBySlug({slug}, 'seminar', page)
    if (data && data?.value) {
...
useHead({
  title: $t('page-titles.info-service.seminar-data')
})
